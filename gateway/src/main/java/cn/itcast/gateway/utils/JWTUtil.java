...
public class JWTUtil {
    ...
    public static void verifyToken(String token, String secretKey) {
        try {
            ...
            JWTVerifier jwtVerifier = JWT.require(Algorithm.HMAC256(secretKey)).build(); // 创建验证对象
            DecodedJWT verify = jwtVerifier.verify(token);
            System.out.println("verifyToken success:" + verify.getClaim(USER_INFO_KEY).asString());
